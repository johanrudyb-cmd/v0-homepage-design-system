...
const EXCLUDE_UNDERWEAR_KEYWORDS = [
  ...
  'soutien gorge', 'brassiere', 'pyjama ', 'nuisette',
];
              ...
              try {
                ...
                const productData = await page.evaluate(`
                    (function() {
                      ...
                      // Extraire la marque depuis le h2 ou le lien marque Zalando (au-dessus du h1)
                      var brandEl = mainContent.querySelector('h2[class*="brand"], a[class*="brand"], [data-testid="brand-name"], [class*="BrandName"], [class*="brandName"]');
                      if (brandEl && !inExclude(brandEl)) productBrand = (brandEl.textContent || '').trim();
                      if (!productBrand && h1) {
                        var prev = h1.previousElementSibling;
                        if (prev && (prev.tagName === 'H2' || prev.tagName === 'SPAN' || prev.tagName === 'A') && (prev.textContent || '').trim().length < 60) {
                          var cand = (prev.textContent || '').trim();
                          if (!/^[\d\u20ac%+]/.test(cand) && cand.length > 1) productBrand = cand;
                        }
                        if (!productBrand && h1.parentElement) {
                          var parentPrev = h1.parentElement.previousElementSibling;
                          if (parentPrev && (parentPrev.textContent || '').trim().length < 60) {
                            var cand2 = (parentPrev.textContent || '').trim();
                            if (!/^[\d\u20ac%+]/.test(cand2) && cand2.length > 1) productBrand = cand2;
                          }
                        }
                      }
                      // Separer le nom du type de vetement (ex: "LINEAR HERITAGE HALF ZIP - Sweatshirt")
                      var category = '';
                      var dashIdx = name.lastIndexOf(' - ');
                      if (dashIdx > 0) {
                        var potentialCategory = name.slice(dashIdx + 3).trim();
                        var potentialName = name.slice(0, dashIdx).trim();
                        if (potentialCategory.split(' ').length <= 5 && potentialCategory.length < 50) {
                      ...
                      if (imageUrl && imageUrl.indexOf('//') === 0) imageUrl = 'https:' + imageUrl;
                      return { name: name.slice(0, 200), price: price, imageUrl: imageUrl, productBrand: productBrand.slice(0, 100), category: category.slice(0, 100) };
                    })()
                  `) as { name: string; price: number; imageUrl: string | null; productBrand: string; category: string };
                ...
                // Marque extraite depuis la page produit (h2 Zalando) ou depuis le nom collé
                const scrapedBrand = productData.productBrand?.trim() || null;
                const finalCategory = productData.category?.trim() || null;
                if (!isExcludedProduct(finalName, source.excludeKeywords)) {
                  enriched.push({
                    ...
                    productBrand: scrapedBrand || card.productBrand || null,
                  });
                }
                await page.goto(listUrl, { waitUntil: 'load', timeout: 25000 });
                await new Promise((r) => setTimeout(r, 2000));
                ...
                (marketZone: string, brand: string, seg: string) => {
                  ...
                  if (brand === 'Zalando') {
                    ...
                    if (!productBrand && name) {
                      ...
                      // Vérifier si la première partie commence par une marque connue
                      const knownBrands = [
                        ...
                        'Timberland', 'Adidas', 'AllSaints', 'Arcteryx', 'Asics', 'Autry', 'Ba&sh', 'Balenciaga',
                        'Barbour', 'Bershka', 'Champion', 'Columbia', 'Converse', 'Diesel', 'Ellesse', 'Ferragamo',
                        'Givenchy', 'Gant', 'Gucci', 'Hackett', 'Hugo Boss', 'Jacquemus', 'Jordan', 'Kappa', 'Kenzo',
                        "Lacoste", "Levi's", 'Levis', 'Loewe', 'Maje', 'Mango', 'Miu Miu', 'Moncler', 'Moschino',
                        'Napapijri', 'Nike', 'Off-White', 'Patagonia', 'Paul Smith', 'Prada', 'Puma', 'Reebok',
                        'Reiss', 'Rick Owens', 'Sandro', 'Schott', 'Supreme', 'Stüssy', 'The Kooples', 'Uniqlo',
                        'Valentino', 'Vans', 'Versace', 'Veja', 'Celine', 'Dior', 'Fendi', 'Fila', 'Gap',
                        'Weekday', 'Terranova', 'Pinko', 'Marella', 'Mother', 'Les Deux', 'Han Kjøbenhavn',
                        'Elisabetta Franchi', 'MM6 Maison Margiela', 'Maison Margiela', 'Umbro', '032c',
                        'Carhartt', 'Dickies', 'Guess', 'Dr. Martens', 'Superdry', 'Burberry', 'Armani',
                        "aim'n", 'Claudie Pierlot', 'Maje', 'Sandro', 'A.P.C.', 'Ami', 'Iro',
                      ].sort((a, b) => b.length - a.length);

                      const firstPartLower = firstPart.toLowerCase();
                      let foundBrand: string | null = null;
                      for (const brand of knownBrands) {
                        if (firstPartLower.startsWith(brand.toLowerCase())) {
                          foundBrand = brand; // Utiliser la casse propre de notre liste
                          break;
                        }
                      }
                      ...
                      } else if (firstPart.length >= 2 && firstPart.length <= 35 && !/sweat|t-shirt|pull|veste|pantalon|short|robe|hoodie|trench|manteau|jean|denim|shirt|top|blouson|cargo|chemise|jupe|polo|gilet|cardigan|blazer|doudoune/i.test(firstPart)) {
                        // Pas une marque connue mais ressemble à un nom propre (pas un terme générique)
                        productBrand = firstPart.slice(0, 100);
                      }
                    }
                  }
                  return {
    ...
    const productLinks = await page.evaluate(`
      (function() {
        function getImgUrl(el) {
          ...
          var d = el.getAttribute('data-src') || el.getAttribute('data-lazy-src') || el.getAttribute('data-image-url') || el.getAttribute('data-srcset');
          if (d && d.indexOf('http') === 0 && !badRe.test(d)) return d;
          if (d && d.indexOf('//') === 0) return 'https:' + d;
          var set = el.getAttribute('srcset') || el.getAttribute('data-srcset');
          if (set) {
      ...
      try {
        ...
        const enriched = await page.evaluate(
          (marketZone: string, brand: string, seg: string) => {
            ...
            // Parser le format ASOS : "Marque - Nom du vêtement - Couleur"
            ...
            const GENERIC_GARMENT_RE = /^(sweat|t-shirt|pull|veste|pantalon|short|robe|hoodie|trench|manteau|jean|denim|shirt|top|blouson|cargo|chemise|jupe|polo|gilet|cardigan|blazer|doudoune|haut|bas|débardeur|debardeur|legging|body|ensemble|costume|maillot|bikini|bermuda|pyjama|jogging|survêtement|survetement)$/i;
            ...
            if (parts.length >= 2) {
              ...
              const lastIsColor = COLOR_RE.test(lastPart);
              const firstIsGeneric = GENERIC_GARMENT_RE.test(firstPart);
              const firstIsAsos = /^asos/i.test(firstPart);
              const firstIsBrand = !firstIsGeneric && !firstIsAsos && firstPart.length <= 40;

              if ((firstIsBrand || firstIsAsos) && parts.length >= 3) {
                // "Marque - Nom - Couleur" ou "ASOS DESIGN - Nom - Couleur"
                const middleParts = lastIsColor ? parts.slice(1, -1) : parts.slice(1);
                name = middleParts.join(' ').trim();
              } else if (firstIsBrand && parts.length === 2 && !lastIsColor) {
                // "Marque - Nom"
                name = lastPart;
              } else if (!firstIsBrand && lastIsColor) {
                // "Nom - Couleur"
                name = parts.slice(0, -1).join(' ').trim();
              }
            }

            const name2 = (name || rawTitle).slice(0, 200);
            ...
            let productBrand: string | null = null;
            // 1. Essayer les métadonnées (le plus fiable)
            const metaBrand = (document.querySelector('meta[property="product:brand"], meta[name="product:brand"], meta[itemprop="brand"]') as HTMLMetaElement)?.content?.trim();
            if (metaBrand && !/asos design|asos edition|asos luxe|asos 4505/i.test(metaBrand)) {
              productBrand = metaBrand.slice(0, 100);
            }
            // 2. Essayer les éléments DOM spécifiques ASOS
            if (!productBrand) {
              const brandEl = document.querySelector('[data-test-id="product-brand"], [class*="brandName"], [class*="brand-name"], a[href*="/brand/"]');
              const brandText = (brandEl as HTMLElement)?.textContent?.trim();
              if (brandText && brandText.length >= 2 && brandText.length <= 60 && !/asos design|asos edition|asos luxe|asos 4505/i.test(brandText)) {
                productBrand = brandText.slice(0, 100);
              }
            }
            // 3. Fallback : chercher dans le titre COMPLET (rawTitle) si ça commence par une marque connue
            if (!productBrand && rawTitle) {
              const knownBrandsAsos = [
                'Alexander McQueen', 'Alpha Industries', 'Bottega Veneta', 'Calvin Klein', 'Canada Goose',
                'Dolce & Gabbana', 'Massimo Dutti', 'New Balance', 'Ralph Lauren', 'Saint Laurent',
                'Stone Island', 'The North Face', 'Tommy Hilfiger', 'Under Armour', 'Zadig & Voltaire',
                'Acne Studios', 'Axel Arigato', 'Birkenstock', 'CP Company', 'Fred Perry', 'Heron Preston',
                ...
                'Elisabetta Franchi', 'MM6 Maison Margiela', 'Maison Margiela', 'Umbro', '032c',
                'Carhartt', 'Dickies', 'Guess', 'Dr. Martens', 'Superdry', 'Burberry', 'Armani',
                "aim'n", 'Claudie Pierlot', 'A.P.C.', 'Ami', 'Iro',
              ].sort((a, b) => b.length - a.length);
              // Utiliser le rawTitle complet pour trouver la marque au début
              const titleParts = rawTitle.split(/\s*[-–]\s*/);
              const firstPart = titleParts[0]?.trim() || '';
              const firstPartLower = firstPart.toLowerCase();
              for (const b of knownBrandsAsos) {
                if (firstPartLower === b.toLowerCase() || firstPartLower.startsWith(b.toLowerCase())) {
                  productBrand = b;
                  break;
                }
              }
              if (!productBrand && titleParts.length >= 2) {
                const first = titleParts[0].trim();
                // Si pas générique et longueur raisonnable -> c'est probablement la marque
                if (first.length > 1 && first.length < 50 && !GENERIC_GARMENT_RE.test(first) && !/asos design|asos edition|asos luxe|asos 4505|asos curve/i.test(first) && !COLOR_RE.test(first)) {
                  productBrand = first;
                }
              }
            }
            const priceText = getText('[data-test-id="current-price"]') || getText('[class*="current-price"]') || getText('[class*="price_"]') || '';
            const priceVal = parseFloat(priceText.replace(/[^\d,.]/g, '').replace(',', '.')) || 0;
...
export async function scrapeHybridSource(
  ...
  try {
    ...
    console.log(`[Scraper] Navigating to ${fullUrl}...`);
    await page.goto(fullUrl, {
      waitUntil: 'domcontentloaded',
      timeout: 45000,
    });
    console.log(`[Scraper] Page landed successfully.`);

    // Gestion bannière cookies (OneTrust / Zara / ASOS)
    try {
      await new Promise((r) => setTimeout(r, 3000));
      const okButton = await page.$('#onetrust-accept-btn-handler, #onetrust-accept-btn, .onetrust-accept-btn-handler');
      if (okButton) {
        console.log(`[Scraper] Cookie banner detected, clicking OK...`);
        await okButton.click();
        await new Promise((r) => setTimeout(r, 2000));
      }
    } catch (e) {
      // Ignorer si pas de bannière
    }
    ...
    if (preScrollSteps > 0) {
      ...
      const scrollTo = new Function('y', 'window.scrollTo(0, y);') as (y: number) => void;
      for (let s = 1; s <= preScrollSteps; s++) {
        console.log(`[Scraper] Pre-scroll step ${s}/${preScrollSteps}...`);
        const maxH = await page.evaluate(getHeight);
        await page.evaluate(scrollTo, Math.round((maxH * s) / preScrollSteps));
        await new Promise((r) => setTimeout(r, 2500));
      }
      await new Promise((r) => setTimeout(r, 2000));
      console.log(`[Scraper] Pre-scroll steps completed.`);
    }
    ...
    const extractProductsFn = `
      ...
      console.log('--- [Scraper] Found ' + products.length + ' products with selector: ' + prodSel);
      ...
      return products.map(function(el) {
        ...
        // Sur ASOS, la carte a 2 lignes : marque (1er p) + nom produit (2e p)
        var cardBrand = null;
        var productName = '';
        var promoRe = /plus de couleurs|\\bça part vite\\b|\\bca part vite\\b|\\bnouveau\\b|\\bnouveaut[ée]\\b|\\bexclusivit[ée]\\b|asos design|asos edition|asos luxe|asos 4505|asos curve|asos white|\\basos\\b|\\bzalando\\b|\\bzara\\b|\\bsale\\b|\\bnew\\b|\\bhot\\b|\\btrending\\b/gi;
        var colorSuffixRe = /\\s*[-–]\\s*(noir|blanc|bleu|rouge|vert|gris|beige|rose|marron|orange|violet|jaune|kaki|indigo|navy|cream|olive|black|white|blue|red|green|grey|gray|pink|brown|khaki|stone|sand|natural|naturel|anthracite|charcoal|nude|ivoire|ivory|gold|silver|dor[ée]|argent[ée]|bordeaux|terracotta|rouille|rust|moutarde|lilas|lavande|menthe|mint|saumon|salmon|p[eê]che|turquoise|cyan|magenta|fuchsia|prune|plum|aubergine|chocolat|caramel|cognac|tabac|army|militaire|sauge|sage|eucalyptus|citron|lemon|lime|corail|abricot|apricot|framboise|cerise|fraise|myrtille|mauve|parme|ardoise|slate|acier|steel|platine|champagne|camouflage|camo|washed|d[ée]lav[ée]|vintage|stonewash)(\\s*\\/\\s*[a-zA-Z\u00C0-\u00FF]+)*$/gi;
        var internalBrandRe = /^(adibreak|firebird|phoenix|jdy|lioness|vero moda|only|pieces|noisy may|object|selected|jack & jones|vila|name it|collusion|reclaimed vintage|4505|design|edition|luxe|curve)$/i;
        
        // Essayer d'abord les sélecteurs ASOS spécifiques pour marque + nom séparés
        var brandEl = el.querySelector('[class*="brandName"], [class*="brand-name"], [class*="productBrand"], [data-test-id="product-brand"]');
        var productNameEl = el.querySelector('[class*="productDescription"], [class*="productName"], [class*="product-name"]');
        
        if (brandEl && productNameEl) {
          // Cas idéal : deux éléments distincts
          var bText = (brandEl.textContent || '').trim();
          var nText = (productNameEl.textContent || '').trim();
          if (bText && !internalBrandRe.test(bText) && !/^asos/i.test(bText)) cardBrand = bText.slice(0, 100);
          productName = nText.replace(promoRe, '').replace(colorSuffixRe, '').replace(/\s{2,}/g, ' ').trim().slice(0, 200);
        } else {
          // Fallback : essayer de parser le titre complet "Marque - Nom"
          var rawName = (nameEl && nameEl.textContent ? nameEl.textContent : '').trim().slice(0, 500);
          var parts = rawName.split(/\s*[-–]\s*/);
          var foundSplitBrand = false;

          if (parts.length >= 2) {
             var first = parts[0].trim();
             // Si pas générique (ASOS DESIGN, etc.) et longueur raisonnable -> c'est probablement la marque
             if (first.length > 1 && first.length < 50 && !internalBrandRe.test(first) && !/^asos/i.test(first) && !promoRe.test(first)) {
                 cardBrand = first;
                 // On prend le reste comme nom, en enlevant la couleur à la fin si présente
                 var rest = parts.slice(1).join(' - ');
                 productName = rest.replace(colorSuffixRe, '').replace(/\s{2,}/g, ' ').trim().slice(0, 200);
                 foundSplitBrand = true;
             }
          }

          if (!foundSplitBrand) {
              // Fallback original : prendre les paragraphes dans la zone productInfo
              var infoEl = el.querySelector('div[class*="productInfo"], div[class*="product-info"], div[class*="productDetails"]') || el;
              var paragraphs = Array.from(infoEl.querySelectorAll('p, span[class*="name"], span[class*="brand"]'));
              // Filtrer les paragraphes vides ou trop longs (prix, promo)
              var textParagraphs = paragraphs.map(function(p) { return (p.textContent || '').trim(); }).filter(function(t) { return t.length > 1 && t.length < 150 && !/[€$£]/.test(t) && !promoRe.test(t); });
              promoRe.lastIndex = 0; // reset regex
              
              if (textParagraphs.length >= 2) {
                // Premier = marque, deuxième = nom
                var firstText = textParagraphs[0];
                var secondText = textParagraphs[1];
                if (!internalBrandRe.test(firstText) && !/^asos/i.test(firstText) && firstText.length <= 50) {
                  cardBrand = firstText;
                }
                productName = secondText.replace(colorSuffixRe, '').replace(/\s{2,}/g, ' ').trim().slice(0, 200);
              } else if (textParagraphs.length === 1) {
                // Un seul texte → c'est le nom du produit
                productName = textParagraphs[0].replace(colorSuffixRe, '').replace(/\s{2,}/g, ' ').trim().slice(0, 200);
              } else {
                // Dernier recours : nameEl classique (déjà extrait dans rawName)
                productName = rawName.replace(promoRe, '').replace(colorSuffixRe, '').replace(/\s{2,}/g, ' ').trim().slice(0, 200);
              }
          }
        }
        ...
        // Si c'est une marque ASOS ou interne, on retourne null pour filtrer ensuite
        if (!cardBrand && (brand === 'ASOS' || sourceUrl.indexOf('asos.com') !== -1)) {
          // Si on n'a pas trouvé de marque externe sur une source ASOS, on ignore l'article
          return null;
        }

        return { name: productName, price: price, imageUrl: imageUrl, sourceUrl: sourceUrl, marketZone: marketZone, brand: brand, segment: segment, productBrand: cardBrand || null };
      }).filter(function(p) { return p !== null && p.name && p.name.length > 2 && (p.price > 0 || p.imageUrl); });
    `;
    ...
    const collectSteps = 10;
    const collectTarget = Math.min(source.limit + 50, 150);
    ...
    const getUrlFromImgBody = `
      ...
      var products = Array.from(document.querySelectorAll(prodSel)).slice(0, 150);
      return products.map(function(itemEl) {
        var linkEl = itemEl.querySelector('a[href*="/prd/"], a[href*="/p/"]') || itemEl.querySelector('a') || (itemEl.closest && itemEl.closest('a'));
        var sourceUrl = (linkEl && linkEl.href) || '';
        var imageUrl = null;
        var imgs = itemEl.querySelectorAll(imgSel + ', img[src*="asos"], img[data-src*="asos"], img[srcset*="asos"], img');
        for (var i = 0; i < imgs.length; i++) { imageUrl = getImageUrl(imgs[i]); if (imageUrl) break; }
        if (!imageUrl) {
          var pictureSource = itemEl.querySelector('picture source[srcset], picture source[data-srcset]');
          if (pictureSource) {
            var set = pictureSource.getAttribute('srcset') || pictureSource.getAttribute('data-srcset');
            if (set) {
              var first = set.split(',')[0].trim().split(/\\\\s+/)[0];
        ...
        if (!imageUrl) {
          var dataEl = itemEl.querySelector('[data-image-url], [data-src], [data-srcset]');
          if (dataEl) {
            var u = dataEl.getAttribute('data-image-url') || dataEl.getAttribute('data-src') || dataEl.getAttribute('data-srcset');
            if (u && u.indexOf('http') === 0) imageUrl = u;
            else if (u && u.indexOf('//') === 0) imageUrl = 'https:' + u;
    ...
    const scrollSteps = 5;
    const scrollWaitMs = source.brand === 'ASOS' ? 3500 : 2500;
